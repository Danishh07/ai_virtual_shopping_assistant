...
const ProductCard = ({ product }) => {
  // Safe rendering helper for when product data might be incomplete
  const rating = product.rating || 0;
  const id = product._id || 'sample';
      ...
      <div className="bg-white rounded-lg shadow-md overflow-hidden transition hover:shadow-lg">        <div className="h-48 md:h-56 bg-gray-100 relative">
          <img 
            ...
            onError={(e) => {
              ...
              e.target.src = `https://via.placeholder.com/300x400?text=${encodeURIComponent(product.name || 'Product')}`;
            }}
          />
        ...
        <div className="p-4">
          ...
          <div className="flex items-center mt-1">
            <div className="flex items-center">
              ...
              <span className="ml-1 text-xs text-gray-500">{rating ? rating.toFixed(1) : "N/A"}</span>
            </div>
            ...
            {product.discountPrice ? (
              ...
              <span className="text-sm font-bold text-gray-900">₹{product.price || 'N/A'}</span>
            )}
          </div>
